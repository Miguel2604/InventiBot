-- FINAL CORRECTED FIX FOR AUTHENTICATION AND RLS POLICIES
-- Run this in your Supabase SQL Editor
-- This script fixes the phone/phone_number column issue AND the invalid enum value
...
-- 3. Check current enum values for invite_status
SELECT unnest(enum_range(NULL::invite_status)) as valid_invite_status_values;

-- 4. Drop all existing policies to start fresh (avoiding conflicts)
DO $$ 
BEGIN
...
-- 5. Create/ensure profiles table exists (this is what the auth service uses)
...
-- 6. Migrate data from user_profiles to profiles if needed
DO $$
DECLARE
...
-- 8. Create simple, non-recursive policies for profiles table
...
-- 9. Create policies for invites table
...
-- FIXED: Only use valid invite_status enum values: pending, claimed, expired, revoked
...
-- 10. Create policies for buildings and units (read-only for anon)
...
-- 11. If user_profiles exists, add simple policies to prevent recursion
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'user_profiles' AND table_schema = 'public') THEN
        -- Service role has full access
...
-- 12. Verify the policies were created correctly
SELECT 
    schemaname,
    tablename, 
    policyname, 
...
-- 13. Create or refresh a test invite for testing
...
BEGIN
    ...
    -- Try to get an existing admin user for created_by
    SELECT id INTO admin_user_id 
    FROM user_profiles 
    WHERE role IN ('admin', 'super_admin') 
    LIMIT 1;
    
    -- If no admin user exists, create a dummy one for the invite
    IF admin_user_id IS NULL THEN
        admin_user_id := uuid_generate_v4();
    END IF;
    
    -- Delete existing TEST2024 invite if it exists
    ...
    RAISE NOTICE 'Test invite code TEST2024 created for unit_id: %, building_id: %', test_unit_id, test_building_id;
END $$;

-- 14. Show available invites for testing
SELECT 
    i.login_code,
    i.status,
    i.full_name,
...
-- 15. Test the connection by counting records
SELECT 
...
-- 16. Final verification - check that the profiles table has the right structure
SELECT 
    column_name,
    data_type,
    is_nullable,
